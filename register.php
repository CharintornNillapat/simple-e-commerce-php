...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql = "INSERT INTO users (name, username, password, email, phone_number, address,  role) VALUES (?, ?, ?, ?, ?, ?, 'customer')";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param('ssssss', $name, $username, $password, $email, $phone_number, $address);
    if ($stmt->execute()) {
...
<body class="bg-gray-100 flex items-center justify-center min-h-screen">
    <div class="w-full max-w-md p-6 bg-white rounded-lg shadow-md">
        ...
        <form method="POST" class="space-y-4">
            ...
            <div>
                <textarea name="address" placeholder="Address" required rows="4"
